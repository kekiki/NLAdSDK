...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    ...
    if (textField.ib_currentLength >= textField.maxLength && textField.markedTextRange == nil && range.length == 0){
        return NO;
    }
...
- (void)setMaxLength:(NSInteger)ib_maxLength{
    objc_setAssociatedObject(self, @selector(maxLength), @(ib_maxLength), OBJC_ASSOCIATION_RETAIN);
    [self ib_proxy];
...
- (void)setMinLength:(NSInteger)ib_minLength{
    objc_setAssociatedObject(self, @selector(minLength), @(ib_minLength), OBJC_ASSOCIATION_RETAIN);
}
...
- (NSInteger)getRemainTextLength{
    return self.maxLength - self.ib_currentLength;
}

- (BOOL)isTextValide{
    return self.ib_currentLength >= self.minLength && self.ib_currentLength <= self.maxLength;
}

- (void)addLeftSpace:(CGFloat)space {
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, space, 40)];
